...
import { Text, View, Dimensions, StyleSheet, TouchableOpacity, Image,TextInput, ScrollView, Pressable   } from 'react-native';
import {
  LineChart,
  BarChart,
  PieChart,
...
export default function Details (props) {
    ...
    function clickMe () {
      fetch("http://192.168.8.20:8080/user/userWallet").then(res => {
        return res.json()
      }).then(data => {
        data.map(i => {
          if(i.f_uid == user.uid){
            ...
            fetch("http://192.168.8.20:8080/wallet/add", {
              method : 'POST',
              headers : {
      ...
      fetch("http://192.168.8.20:8080/user/userWallet").then(res => {
        ...
        data.map(i => {
          if(i.f_uid == user.uid){
            //console.log(i)
            //console.log(i.id+" "+name+" "+val+" "+val*priceUsd)
            fetch("http://192.168.8.20:8080/wallet/sell", {
              method : 'POST',
              headers : {
                'Accept': 'application/json',
                'Content-Type' : 'application/json'
        ...
        fetch("http://192.168.8.20:8080/user/userWallet").then(res => {
          ...
          data.map(i => {
            if(i.f_uid == user.uid){
              setSolde(i.solde)
              //console.log(i)
              fetch("http://192.168.8.20:8080/wallet/allWallet").then(res => {
                return res.json()
              }).then(info => {
                info.map(o => {
                  if(o.id_user == i.id){
    ...
    return (
        <ScrollView>
            ...
            <View style={styles.container}>
                ...
                <Pressable style={styles.button} onPress={sellCurr}>
                ...
                chartConfig={{
                  ...
                  color: (opacity = 1) => `rgba(158,44,58, ${opacity})`,
                  labelColor: (opacity = 1) => `rgba(158,44,58, ${opacity})`,
                  style: {
                    borderRadius: 16
                  },
                  propsForDots: {
